...
func TestRedisCreateSession(t *testing.T) {
	...
	_, _, err := r.CreateSession("1", "test@test.com", "fullname", "hash", "csrfToken", time.Now(), time.Now(), false, "selector", "token", time.Now(), time.Now())
	if err == nil || len(m.QueriesRun()) != 0 {
		t.Error("expected error")
	}

	// expired rememberMe, but session should save.
	_, _, err = r.CreateSession("1", "test@test.com", "fullname", "hash", "csrfToken", time.Now(), time.Now().AddDate(1, 0, 0), true, "selector", "token", time.Now(), time.Now())
	...
	m = onedb.NewMock(nil, nil, nil)
	r = backendRedisSession{db: m, prefix: "test"}
	session, rememberMe, err := r.CreateSession("1", "test@test.com", "fullname", "hash", "csrfToken", time.Now(), time.Now().AddDate(1, 0, 0), true, "selector", "token", time.Now(), time.Now().AddDate(1, 0, 0))
